...
class VirtualKeyboard:
    ...
    def check_deletion_gesture(self, hand_landmarks):
        ...
        all_fingers_bent = all(
            tip.y < hand_landmarks.landmark[idx + 1].y 
    ...
    def process_hand_interaction(self, hand_landmarks):
        ...
        if self.check_deletion_gesture(hand_landmarks):
            ...
            # Continuous deletion after a delay
            if current_time - hand_state['deletion_start_time'] > 0.5:
                if self.current_text:
                    self.current_text = self.current_text[:-1]
                    # Optional: add to history for undo functionality
