...
class VirtualKeyboard:
    def __init__(self):
        ...
        self.deletion_start_time = 0
        self.deletion_cooldown = 0
    ...
    def check_click(self, landmarks):
        # Advanced click detection with multiple gestures
    ...
    def check_deletion_gesture(self, landmarks):
        # Advanced deletion gesture detection
        ...
        all_fingers_bent = all(
            tip.y < landmarks.landmark[idx + 1].y 
            for idx, tip in zip([7, 11, 15, 19], tips)
        )
    ...
    def draw_keyboard(self, frame):
        for button in self.buttons:
            ...
            if self.current_landmarks and self.current_landmarks.multi_hand_landmarks:
                for hand_landmarks in self.current_landmarks.multi_hand_landmarks:
    ...
    def process_key_press(self):
        ...
        for hand_landmarks in self.current_landmarks.multi_hand_landmarks:
            # Deletion gesture check
            if self.check_deletion_gesture(hand_landmarks):
                if not self.deletion_mode:
                    self.deletion_mode = True
                    self.deletion_start_time = cv2.getTickCount()
                
                # Continuous deletion after a short delay
                current_time = cv2.getTickCount()
                if (current_time - self.deletion_start_time) / cv2.getTickFrequency() > 0.5:
                    if self.deletion_cooldown <= 0:
                        # Delete last character
                        if self.current_text:
                            self.current_text = self.current_text[:-1]
                        self.deletion_cooldown = 5  # Cooldown to prevent rapid deletion
            else:
                self.deletion_mode = False
                self.deletion_cooldown = max(0, self.deletion_cooldown - 1)
